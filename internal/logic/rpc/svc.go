...
func (s *logicService) Register(ctx context.Context, req *logic.RouterReq) (*logic.RouterResp, error) {
	onlineVal := s.buildRouterVal(ctx, req)
	err := global.Redis.SAdd(ctx, fmt.Sprintf(cacheOnlineUid, req.Uid), onlineVal).Err()
	if err != nil {
		return nil, err
	}
	return &logic.RouterResp{Code: 0}, nil
}

func (s *logicService) UnRegister(ctx context.Context, req *logic.RouterReq) (*logic.RouterResp, error) {
	onlineVal := s.buildRouterVal(ctx, req)
...
func (*logicService) buildRouterVal(_ context.Context, req *logic.RouterReq) string {
